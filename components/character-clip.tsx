...
export function CharacterClip({clip}:{clip:NonNullable<Game['characterClip']>}){
 ...
 useEffect(()=>{
  const el=video.current;if(!el)return;let cancelled=false;
  const play=async()=>{el.muted=!sound.current;try{await el.play();if(!cancelled)setNeedsTap(false)}catch{if(cancelled)return;el.muted=true;if(sound.current)setNeedsTap(true);void el.play().catch(()=>{})}};
  ...
  const start=()=>{el.currentTime=Math.max(0,(Date.now()-clip.at)/1000);void play()};
  read();el.muted=!sound.current;
  window.addEventListener('sj-feedback-change',changed);window.addEventListener('storage',changed);
  el.addEventListener('loadedmetadata',start);if(el.readyState>=1)start();
  return()=>{cancelled=true;window.removeEventListener('sj-feedback-change',changed);window.removeEventListener('storage',changed);el.removeEventListener('loadedmetadata',start);el.pause()};
 },[clip.id,clip.at]);
 ...
 return <div className={'character-clip'+(ready?' clip-ready':'')} role="status"><video ref={video} src={'/character-clips/'+clip.role+'.mp4'} poster={'/cards/'+clip.role+'.webp'} onPlaying={()=>setReady(true)} playsInline preload="auto" aria-label={card(clip.role).name+' চরিত্রের ভিডিও'}/><div><strong>{clip.actor}</strong><span>{card(clip.role).name} দাবি করেছে</span></div>{needsTap&&<button type="button" className="clip-sound-button" onClick={enableSound}>শব্দ শুনতে চাপুন</button>}</div>;
}
