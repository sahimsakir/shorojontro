...
for(const [role,duration] of [['nantu',4100],['brahma',3500],['betal',3200]])test(role+' pauses presentation and preserves the full response window',()=>{
 ...
 beginCharacterClip(g,4,1000);assert.equal(g.characterClip.role,role);assert.equal(g.waiting.deadline,26000+duration+650);assert.equal(clipPlaying(g,1001),true);assert.equal(clipPlaying(g,1000+duration+650),false);
