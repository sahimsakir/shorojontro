...
for(const name of ['cards','bots','engine']){const s=readFileSync(`lib/game/${name}.ts`,'utf8').replace("'./cards'","'./cards.mjs'").replace("'./bots'","'./bots.mjs'");writeFileSync(`.sites-runtime/engine-tests/${name}.mjs`,ts.transpileModule(s,{compilerOptions:{target:ts.ScriptTarget.ES2022,module:ts.ModuleKind.ES2022}}).outputText)}
